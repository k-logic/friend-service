...
async def submit_verification(
    ...
    existing = await db.execute(
        select(AgeVerification).where(
            AgeVerification.user_id == user.id,
    ...
    verification = AgeVerification(user_id=user.id)
    db.add(verification)
    await db.commit()
...
async def list_verifications(
    ...
    name_map = await get_display_name_map(db, [v.user_id for v in items])
    return [
        AgeVerificationResponse.model_validate(v, from_attributes=True).model_copy(
            update={"user_display_name": name_map.get(v.user_id)}
        )
        for v in items
    ]
...
async def review_verification(
    ...
    await db.commit()
    await db.refresh(verification)
    name_map = await get_display_name_map(db, [verification.user_id])
    return AgeVerificationResponse.model_validate(verification, from_attributes=True).model_copy(
        update={"user_display_name": name_map.get(verification.user_id)}
    )
