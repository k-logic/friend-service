...
async def list_templates(
    ...
    stmt = select(Template).where(Template.staff_id == staff.id).order_by(Template.label)
...
async def create_template(
    ...
    template = Template(staff_id=staff.id, label=body.label, content=body.content)
    db.add(template)
    await db.commit()
...
async def update_template(
    ...
    if template.staff_id != staff.id and staff.role != StaffRole.admin:
