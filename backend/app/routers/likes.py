...
async def send_like(
    ...
    existing = await db.execute(
        select(Like).where(Like.user_account_id == account.id, Like.persona_id == body.persona_id)
    )
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="既にいいね済みです")

    like = Like(user_account_id=account.id, persona_id=body.persona_id)
    db.add(like)
    await db.commit()
...
async def remove_like(
    ...
    result = await db.execute(
        delete(Like).where(Like.user_account_id == account.id, Like.persona_id == persona_id)
    )
...
async def list_likes(
    ...
    stmt = select(Like).where(Like.user_account_id == account.id).order_by(Like.created_at.desc())
